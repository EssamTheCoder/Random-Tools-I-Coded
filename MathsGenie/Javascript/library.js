// All the funky functions (and variables on the side)

let theme = 1;
export function changeTheme() {
    const root = document.querySelector(`:root`).style;
    const themeButton = document.getElementById(`themeButton`);
    if(theme == 1) { //Light -> Dark
        theme = 2;
...
export async function getSpecific(grade,attribute,index) {
    const processing = await get(grade,attribute,false);

    return processing[index];
}

export async function intialiseCheckboxes() {
    for(let i = 1; i <= 9; i++) {
